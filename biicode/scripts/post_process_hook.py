...
def prepare_all_samples():
    ...
    for module in os.listdir(root_folder):
        if (platform.system() != "Windows" and module == "NetSSL_Win") \
           or module in ignored_samples:
            continue
        samples_module_path = os.path.join(module, 'samples')
    ...
    # Foundation
    !examples/Foundation/BinaryReaderWriter/*
    !examples/Foundation/LineEndingConverter/*
    # Crypto
...
def apply_changes():
    ...
    search_and_replace(os.path.join("Foundation", "src", "Checksum.cpp"), "Poco/zlib.h", "zlib/zlib/zlib.h")
    search_and_replace(os.path.join("Zip", "src", "ZipStream.cpp"), "Poco/zlib.h", "zlib/zlib/zlib.h")
...
def revert_changes():
    ...
    search_and_replace(os.path.join("Foundation", "src", "Checksum.cpp"), "zlib/zlib/zlib.h", "Poco/zlib.h")
    search_and_replace(os.path.join("Zip", "src", "ZipStream.cpp"), "zlib/zlib/zlib.h", "Poco/zlib.h")
    if os_platform == "Windows":
...
cmakelist_token = "set(CMAKE_MODULE_PATH ${CMAKE_MODULE_PATH} ${CMAKE_CURRENT_SOURCE_DIR}/cmake)"
cmakelist_replacement = '''set(CMAKE_MODULE_PATH ${CMAKE_MODULE_PATH} ${CMAKE_CURRENT_SOURCE_DIR}/cmake)
if(BIICODE)
include(biicode/cmake/biicode.cmake)
return()
endif()'''
cmakelist_replacement_win = "set(CMAKE_MODULE_PATH ${CMAKE_MODULE_PATH} ${CMAKE_CURRENT_SOURCE_DIR}/cmake)\r\nif(BIICODE)\r\ninclude(biicode/cmake/biicode.cmake)\r\nreturn()\r\nendif()"
